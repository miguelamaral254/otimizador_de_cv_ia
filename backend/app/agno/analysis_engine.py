"""
Motor de análise refatorado seguindo princípios SOLID.
"""
...
from app.core.interfaces import IAnalysisEngine
...
class TextAnalyzer(ABC):
    """Interface para analisadores de texto."""
    
    @abstractmethod
    def analyze(self, text: str) -> Dict[str, Any]:
        """Analisa o texto e retorna resultados."""
        pass


class QuantificationAnalyzer(TextAnalyzer):
    """Analisador específico para quantificação."""
    
    def __init__(self):
        # Padrões de quantificação avançados
    ...
    def _analyze_with_regex(self, text: str) -> Dict[str, Any]:
        """Análise usando regex como fallback."""
        try:
...
class ActionVerbAnalyzer(TextAnalyzer):
    """Analisador específico para verbos de ação."""
...
class KeywordAnalyzer(TextAnalyzer):
    """Analisador específico para palavras-chave."""
    ...
    def analyze(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'score': min(10, len(found_keywords)),
            'coverage': len(found_keywords) / len(self.tech_keywords)
        }


class TextStructureAnalyzer(TextAnalyzer):
    """Analisador específico para estrutura de texto."""
...
class RefactoredAnalysisEngine(IAnalysisEngine):
    """Motor de análise refatorado seguindo princípios SOLID."""
    
    def __init__(self):
        self.quantification_analyzer = QuantificationAnalyzer()
    ...
    async def analyze_curriculum(self, text: str) -> Dict[str, Any]:
        ...
        try:
            # Análises paralelas usando os analisadores específicos
            quantification = await self.analyze_quantification(text)
            action_verbs = await self.analyze_action_verbs(text)
            keywords = self.keyword_analyzer.analyze(text)
            structure = self.structure_analyzer.analyze(text)
            
            # Calcula pontuação geral
            total_score = (
                quantification.get('score_quantificacao', 0) * 0.3 +
                action_verbs.get('score_verbos', 0) * 0.3 +
                keywords.get('score', 0) * 0.2 +
                structure.get('estrutura_geral', {}).get('score_geral', 0) / 100 * 0.2
            )
    ...
    async def analyze_quantification(self, text: str) -> Dict[str, Any]:
        """Analisa quantificação no texto."""
        return self.quantification_analyzer.analyze(text)
    
    async def analyze_action_verbs(self, text: str) -> Dict[str, Any]:
        """Analisa verbos de ação no texto."""
        return self.action_verb_analyzer.analyze(text)
    
    def _get_overall_assessment(self, score: float) -> str:
        """Retorna avaliação geral baseada na pontuação."""
        if score >= 0.8:
            return 'excellent'
        elif score >= 0.6:
            return 'good'
...
class AnalysisEngine:
    """Motor de análise original para compatibilidade."""
    ...
    async def analyze_curriculum(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calcula pontuação geral
            total_score = (
                quantification.get('score_quantificacao', 0) * 0.5 +
                action_verbs.get('score_verbos', 0) * 0.5
            )
